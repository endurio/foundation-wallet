...
// RPCClient represents a persistent client connection to a endurio RPC server
...
// operate on the same endurio network as described by the passed chain
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err != nil {
		return errors.E(op, errors.E(errors.Op("ndrd.jsonrpc.getcurrentnet"), err))
	}
...
// endurio RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to ndrd processing websocket requests synchronously (see
